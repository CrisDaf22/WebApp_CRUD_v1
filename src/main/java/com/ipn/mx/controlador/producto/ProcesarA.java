...
@WebServlet(name = "ProcesarA", value = "/ProcesarA")
@MultipartConfig(
    fileSizeThreshold = 1024 * 1024 * 1, // 1 MB
    maxFileSize = 1024 * 1024 * 2,      // 2 MB
    maxRequestSize = 1024 * 1024 * 5   // 5 MB
)
public class ProcesarA extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Respuesta actualización</title>"); 
            ...
            try {
                ...
                out.println("<div class='alert alert-success' role='alert'>El producto se actualizó con éxito.</div>");
            } catch (SQLException ex) {
                Logger.getLogger(ProcesarA.class.getName()).log(Level.SEVERE, null, ex);
                out.println("<div class='alert alert-danger' role='alert'>El producto no se pudo actualizar.</div>");
            }
